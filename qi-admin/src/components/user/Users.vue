  ...
  <div>
    ...
    <el-card>
      ...
      <el-table :data="userlist" border stripe>
        ...
        <el-table-column label="角色(1为普通用户，2为管理员)" prop="role"></el-table-column>
    ...
    export default {
        data() {
            ...
            var checkRole = (rule, value, cb) => {
                // 验证角色的正则表达式
                const regMobile = /^[1-2]{1}$/

                if (regMobile.test(value)) {
                    return cb()
                }

                cb(new Error('请输入合法的角色'))
            }
        ...
        methods: {
            ...
            async showEditDialog(id) {
                // console.log(id)
                const {data: res} = await this.$http.get('/queryuser', {params: {'id': id}})
            ...
            editUserInfo() {
                this.$refs.editFormRef.validate(async valid => {
                    ...
                    const {data: res} = await this.$http.get(
                        '/updatauser', {params: this.editForm})
